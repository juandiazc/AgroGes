...
Partial Class Modulo_Gestion_frmCertificadoRecepcion
    ...
    Dim CN As New SqlClient.SqlConnection("Server=LIMONERO;database=COSECHA2015;uid=usuariocosecha2015;pwd=usuariocosecha2015")
    ...
    Sub traer()
        ...
        If drop_sis.SelectedValue = 1 Then
            CONSULTAENC = "SELECT A1.numeroinf, UPPER(CASE WHEN A1.Region=99 THEN 'Total País/P' WHEN A1.Region=1 THEN 'Tarapaca' WHEN A1.Region=15 THEN 'Arica y Parinacota' WHEN A1.Region=2 THEN 'Antofagasta' WHEN A1.Region=3 THEN 'Atacama' WHEN A1.Region=4 THEN 'Coquimbo' WHEN A1.Region=5 THEN 'Valparaíso' WHEN A1.Region=13 THEN 'Metropolitana' WHEN A1.Region=6 THEN 'O'+CHAR(39)+'Higgins' WHEN A1.Region=7 THEN 'Maule' WHEN A1.Region=8 THEN 'Biobío' WHEN A1.Region=9 THEN 'La Araucanía' WHEN A1.Region=14 THEN 'Los Ríos' WHEN A1.Region=10 THEN 'Los Lagos' WHEN A1.Region=11 THEN 'Aysén' WHEN A1.Region=12 THEN 'Magallanes' END) AS Region, A1.nombre, A1.rut, A1.dv_rut, A1.PeriodoDEL,A1.PeriodoAL, A1.Fechainf, A1.M1, A1.E1 FROM(SELECT 1 AS numeroinf, 11 AS Region, UPPER('Camila Villanueva')AS nombre,19461281 AS rut,8 AS dv_rut,'1/Marzo/2015' AS PeriodoDEL,'31/Marzo/2015' AS PeriodoAL,replace(convert(NVARCHAR, getdate(), 106), ' ', '/') AS Fechainf,'LEVANTAMIENTO ENCUESTA DE COSECHA 2015' AS M1,'LA DIGITADORA CAMILA VILLANUEVA HA REALIZADO LA DIGITACIÓN DE 136 ENCUESTAS DE LA REGIÓN XI POR UN MONTO TOTAL DE $ 81.600.' AS E1 UNION SELECT 1 AS numeroinf, 12 AS Region, UPPER('Karem Mery')AS nombre,16142891 AS rut,4 AS dv_rut,'1/Marzo/2015' AS PeriodoDEL,'31/Marzo/2015' AS PeriodoAL,replace(convert(NVARCHAR, getdate(), 106), ' ', '/') AS Fechainf,'LEVANTAMIENTO ENCUESTA DE COSECHA 2015' AS M1,'LA DIGITADORA KAREM MERY HA REALIZADO LA DIGITACIÓN DE 68 ENCUESTAS DE LA REGIÓN XII POR UN MONTO TOTAL DE $ 40.800.' AS E1)AS A1 WHERE A1.numeroinf=1 AND A1.nombre='" + drop_enc.Text + "'"
        ...
        ElseIf drop_sis.SelectedValue = 2 Then
            CONSULTAENC = "SELECT distinct UPPER(gu.NombreUsuario+' ' + gu.ApellidoPaterno+ ' ' + CASE WHEN gu.ApellidoMaterno IS NULL THEN '' ELSE gu.ApellidoMaterno END) AS nombre,replace(convert(NVARCHAR, getdate(), 106), ' ', '-') AS Fechainf,'7-May-2015'AS PeriodoDel,'30-May-2015' AS PeriodoAL,gu.Rut,gu.DvRut,UPPER(CASE WHEN grg.Region=99 THEN 'Total País/P' WHEN grg.Region=1 THEN 'Tarapaca' WHEN grg.Region=15 THEN 'Arica y Parinacota' WHEN grg.Region=2 THEN 'Antofagasta' WHEN grg.Region=3 THEN 'Atacama' WHEN grg.Region=4 THEN 'Coquimbo' WHEN grg.Region=5 THEN 'Valparaíso' WHEN grg.Region=13 THEN 'Metropolitana' WHEN grg.Region=6 THEN 'O'+CHAR(39)+'Higgins' WHEN grg.Region=7 THEN 'Maule' WHEN grg.Region=8 THEN 'Biobío' WHEN grg.Region=9 THEN 'La Araucanía' WHEN grg.Region=14 THEN 'Los Ríos' WHEN grg.Region=10 THEN 'Los Lagos' WHEN grg.Region=11 THEN 'Aysén' WHEN grg.Region=12 THEN 'Magallanes' END) AS Region,'LEVANTAMIENTO ENCUESTA DE COSECHA 2015' AS M1 ,'EL ENCUESTADOR ' + UPPER(gu.NombreUsuario)+' ' + UPPER(gu.ApellidoPaterno)+ ' HA CUMPLIDO SATISFACTORIAMENTE CON LA CARGA ASIGNADA. REALIZÓ '+ CAST(A.N AS VARCHAR(3))+ ' ENCUESTAS ' AS E1,'QUE EQUIVALEN A $'+ CAST(A.M+3786 AS VARCHAR(12)) + ' (INCLUYE SEGURO $3.786).' AS E2 FROM Ges_RecepcionGt grg,Ges_Usuario gu, (SELECT SUM(V1.N)AS N, SUM(V1.M) AS M, V1.Encuestador FROM(SELECT COUNT(V.Id) AS N,(COUNT(V.Id)*V.TipoValor)AS M,V.Encuestador,V.PkSistema FROM (SELECT grg.PkSistema,grg.Id, 12500 AS TipoValor,grg.Encuestador FROM Ges_RecepcionGt grg,Ges_Encuesta ge WHERE ge.Folio=grg.Folio AND grg.PkSistema=ge.PkSistema AND grg.Id=ge.Id AND ge.PkEstadoEncuesta>3 AND ge.PkEstadoSeleccion=1 AND grg.Estado_Pago IS null)AS V  WHERE V.Encuestador='" + drop_enc.Text + "' GROUP BY V.TipoValor, V.Encuestador,V.PkSistema)AS V1  GROUP BY V1.Encuestador	) AS A WHERE grg.PkSistema=gu.PkSistema AND grg.Encuestador=gu.PkUsuario AND A.Encuestador=grg.Encuestador AND grg.Estado_Pago IS NULL AND grg.Encuestador='" + drop_enc.Text + "'"
        ElseIf drop_sis.SelectedValue = 3 Then
        ...
        If drop_sis.SelectedValue = 2 And drop_enc.SelectedValue <> "Seleccione un encuestador" Then
            ...
            txt_rut.Text = exeNUMMAX.Rows(0)(4).ToString
            txt_dvrut.Text = exeNUMMAX.Rows(0)(5).ToString
            txt_region.Text = exeNUMMAX.Rows(0)(6).ToString
            Motivo = exeNUMMAX.Rows(0)(7).ToString
            E1 = exeNUMMAX.Rows(0)(8).ToString
            E2 = exeNUMMAX.Rows(0)(9).ToString
        ...
        If drop_sis.SelectedValue = 2 Then
            If aux = "" Then
                Etodo = E1 + "COSECHA(" + aux2 + ") " + E2
            ElseIf aux2 = "" Then
                Etodo = E1 + "DE COSECHA(" + aux + ") " + E2
            Else
                Etodo = E1 + ": " + n3 + " DE COSECHA(" + aux + ") Y " + n4 + " COSECHA (" + aux2 + ") " + E2
                l1 = Len(E1 + ": " + n3 + " DE COSECHA(" + aux + ") Y ") + n4
                l2 = Len(E1 + ": " + n3 + " DE COSECHA(" + aux + ") Y " + n4 + " COSECHA (" + aux2 + ") " + E2)
            End If
        Else
            Etodo = E1
        End If
        ...
        Dim CONSULTAENC4 As String = "SELECT distinct UPPER(CASE WHEN eb.P1_2='Aisén' THEN 'Aysén' ELSE eb.P1_2 END ) AS ID	FROM Ges_Encuesta ge1, Ges_RecepcionGt grg1,ENC_Cosecha eb	WHERE ge1.PkSistema=grg1.PkSistema AND ge1.Id=grg1.Id AND ge1.Folio=grg1.Folio AND grg1.Estado_Pago IS null  AND ge1.PkEstadoEncuesta>3 AND ge1.PkSistema=eb.PkSistema AND ge1.Id=eb.Id AND ge1.Folio=eb.Folio  AND grg1.Encuestador='" + drop_enc.Text + "'"
        'drop_enc.Text
    ...
    Protected Sub btn_pago_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btn_pago.Click
        Try
            CN.Open()
            Dim CONSULTAENC2 As String = "SELECT  grg1.Id AS IDS FROM Ges_Encuesta ge1, Ges_RecepcionGt grg1	WHERE ge1.PkSistema=grg1.PkSistema AND ge1.Id=grg1.Id AND ge1.Folio=grg1.Folio AND grg1.Estado_Pago IS null  AND ge1.PkEstadoSeleccion=1 AND ge1.PkEstadoEncuesta>3  AND grg1.Encuestador='" + drop_enc.Text + "'"
            'drop_enc.Text
            Dim c1 As Integer
            Dim aux As String
            Dim NUMMAXPES2 As New SqlCommand(CONSULTAENC2, CN)
